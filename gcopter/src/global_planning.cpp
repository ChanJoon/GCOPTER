...
public:
          ...
          visualizer(nh)
    {
        ...
        targetSub = nh.subscribe(config.targetTopic, 1, &GlobalPlanner::targetCallBack, this,
                                 ros::TransportHints().tcpNoDelay());
        trajSub = nh.subscribe("trajectory", 2, &GlobalPlanner::rcvTrajectoryCallabck, this);
        ...
        controlTimer = nh.createTimer(ros::Duration(0.01), &GlobalPlanner::pubPositionCommand, this);
    }
    ...
    inline void pubPositionCommand(const ros::TimerEvent &event)
    {
        ...
        if (state == TRAJ)
        {
            ...
            double t = std::max(0.0, (cmd.header.stamp - _start_time).toSec());
    ...
    inline void targetCallBack(const geometry_msgs::PoseStamped::ConstPtr &msg)
    {
        if (mapInitialized)
        {
            ...
            ROS_INFO("GOAL Selected! x: %f, y: %f, z: %f", goal[0], goal[1], goal[2]);
