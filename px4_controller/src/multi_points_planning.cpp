...
public:
    ...
    inline void plan()
    {
        ...
        {
            std::vector<std::pair<double, std::vector<Eigen::Vector3d>>> indexedRoutes;
						...
						for (unsigned int i = 0; i < startGoal.size() - 1; i++)
						{
							...
							sfc_gen::planPath<voxel_map::VoxelMap>(startGoal[i],
																										...
																										&voxelMap, 0.01,
																										route);
							indexedRoutes.push_back(make_pair(i, route));
            ...
            if (routes.size() > 1)
            {
                ...
                traj.clear();
								...
								// for (int i = 0; i < indexedRoutes.size(); i++)
								// {
								// 	Trajectory<5> pieceTraj;
								// 	Eigen::Matrix3d iniState;
								// 	Eigen::Matrix3d finState;
								// 	iniState.row(0) = indexedRoutes[i].second.front();
								// 	iniState.row(1) = Eigen::Vector3d::Zero();
								// 	iniState.row(2) = Eigen::Vector3d::Zero();

								// 	finState.row(0) = indexedRoutes[i].second.back();
								// 	finState.row(1) = Eigen::Vector3d::Zero();
								// 	finState.row(2) = Eigen::Vector3d::Zero();
								...
								// 	if (std::isinf(gcopter.optimize(pieceTraj, config.relCostTol)))
								// 	{
								// 		return;
								// 	}
								// 	traj.append(pieceTraj);
								// }
								Eigen::Matrix3d iniState;
                Eigen::Matrix3d finState;
